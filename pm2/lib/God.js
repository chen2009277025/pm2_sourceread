 ...
 */
readLog('重载cluster 模块的配置，给process挂载一些方法')
cluster.setupMaster({
...
var God = module.exports = {
  ...
  clusters_db : {},
  started_at : Date.now(),
  bus : new EventEmitter2({ // 事件机制，更优性能的EventEmitter
    wildcard: true,
    delimiter: ':',
    maxListeners: 1000
  })
};
// 重载console 用emit触发到bus(eventemitter)上执行对应打印方法
...
require('./God/ForkMode.js')(God); // fork方式
require('./God/ClusterMode.js')(God); // cluster模式
...
God.executeApp = function executeApp(env, cb) {
  ...
  if (env_copy['pm_id'] === undefined) {
    readLog('如果进程id是空的，初始化进程启动时间')
    env_copy['pm_id']             = God.getNewId(); // 产生一个新的PID
  ...
  var readyCb = function ready(proc) {
      // 如果启用了版本比较，则会调用vizion比较当前代码是否和git仓库一致
...
God.prepare = function prepare (env, cb) {
  ...
  // timesLimit 最大并行执行，这里限制了1，也就是依次的执行
